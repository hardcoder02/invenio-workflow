...
public class CCDetailTableModel extends AbstractTableModel implements TableModel {

	private final List<CCMatrixCell> cells = new ArrayList<CCMatrixCell>();
	
	public CCDetailTableModel() {
	}
	
	public void setCells(List<CCMatrixCell> newCells) {
		cells.clear();
	...
	public Object getValueAt(int rowIndex, int colIndex) {
		...
		CCBaseEntry entry = getBaseEntry(rowIndex);
		switch (colIndex) {
	...
	public CCBaseEntry getBaseEntry(int index) {
		for (CCMatrixCell c : cells) {
			if ( index < c.size() )
				return c.get(index);
			else
