package invenio.op.operation.impl;

import invenio.data.InvenioElement;
import invenio.operator.data.Feature;
import invenio.operator.data.FeatureDescriptor;
import invenio.operator.data.NumericFeature;
import qng.client.QueryException;
import qng.core.query.AbstractOperation;
import qng.core.query.CompiledOperation;

// TODO: hint regarding conf attribute name
public class BinOperation extends AbstractOperation implements
		CompiledOperation {
	
	public final static String OP_NAME = "bin";
	public final static double DEFAULT_THRESHOLD = 0.5;
	public final static String CONF_ATTRIB_NAME = "conf";
	
	public static final Class[] EXPECTED_TYPES = {
		InvenioElement.class
	};
	
	public BinOperation() {
		super(null, OP_NAME);
	}
	
	public BinOperation(String id) {
		super(id, OP_NAME);
	}
	
	@Override
	public Class[] getExpectedTypes() {
		return EXPECTED_TYPES;
	}
	
	@Override
	public boolean hasExactNumberOfArguments() {
		return false;
	}

	@Override
	public Object doExecute(Object[] args) throws QueryException {
		if ( args[0] == null)
			return null;
		
		InvenioElement e = (InvenioElement) args[0];
		
		double threshold = DEFAULT_THRESHOLD;
		if ( args.length > 1) {
			if ( args[1] == null )
				return null;
			if ( !(args[1] instanceof Double) )
				throw new QueryException("Error executing operation [" + getName() + "], id[" + getId() + 
						"]. Expected argument 1 is of type Double, actual type is " + args[1].getClass().getName() );
			threshold = (Double) args[1];
		}
		
		String confAttrName = OperatorUtils.retrieveContextParam(getContext(), OperationConstants.BIN_CONF_NAME, CONF_ATTRIB_NAME);
		
		Feature<? extends FeatureDescriptor> f = OperatorUtils.getFeature(e, confAttrName);
		
		if (f == null)
			return null;
		else if (f instanceof NumericFeature) {
			NumericFeature nf = (NumericFeature)f;
			return bin(nf.getValue(), threshold);
		}
		else {
			throw new QueryException("Error executing operation [" + getName() + "], id[" + getId() + 
					"]. Confidence attribute must be of numeric type");
		}
	}

	private boolean bin(double value, double threshold) throws QueryException {
		if (value < 0 || value > 1)
			throw new QueryException("Error executing operation [" + getName() + "], id[" + getId() + 
					"]. Illegal confidence value " + value );
		
		return( value > threshold );
	}
	
}
