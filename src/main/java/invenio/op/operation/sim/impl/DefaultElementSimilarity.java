...
public class DefaultElementSimilarity extends AbstractElementSimilarity {
	
	public final static String OP_NAME = "defaultElementSimilarity";
	
	public DefaultElementSimilarity() {
		super(null, OP_NAME);
	}
	
	public DefaultElementSimilarity(String id) {
		super(id, OP_NAME);
	}
		...
		for ( int i = 0; i < s1.size(); i++ ) {
			if ( s1.canGetAsCategorical( i ) ) {
				try {
					...
					CompiledOperation delegateSim = reg.createCompiledOperation( SimilarityConstants.DEFAULT_SIM );
