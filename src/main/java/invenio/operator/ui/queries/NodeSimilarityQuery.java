...
/**
 * select g1Node, g2Node, sim(g1Node.location, g2Node.location, {measure=KLDivergence})
 * from (
 * (select g1Node
 ...
 *
 */
public class NodeSimilarityQuery {
	...
	public static LogicalExpression createSelectOuter() {
		...
		select.addOperand( new LogicalConstant("select1", createSimExpression()) );
		select.addOperand( new LogicalConstant("select1alias", "sim"));
		
		return select;
	}
	
	public static LogicalExpression createSimExpression() {
		LogicalOperator expr = new LogicalOperatorImpl("sim");
		
		LogicalOperator path1 = new LogicalOperatorImpl("path");
		path1.addOperand( new LogicalConstant("column", "g1Node") );
		...
		path2.addOperand( new LogicalConstant("column", "g2Node") );
		path2.addOperand( new LogicalConstant("attribute", "location") );
